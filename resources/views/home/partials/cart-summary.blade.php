    ...
    <div class="max-h-[300px] overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        ...
        @else
            <div class="flex items-center justify-center min-h-[120px] bg-gray-50 border border-dashed border-gray-300">
                <p class="text-sm text-black text-center p-4">Your cart is empty. Add a room or boat to start booking!
                </p>
            </div>
        @endif
    ...
    <div class="mt-6 pt-4 border-t border-gray-200">
        ...
        @if($cartRooms->count() || $cartBoats->count())
            <a href="{{ route('checkout.show', ['room_id' => $cartRooms->first()->id ?? 0]) }}"
                class="w-full text-sm block bg-[#964B00] hover:bg-black px-10 py-3 text-white transition text-center shadow-lg">
                Proceed to Checkout
            </a>
        @else
            <button
                class="w-full text-sm bg-[#964B00] text-white mt-5 py-3  cursor-not-allowed  "
                disabled>
                Checkout
            </button>
        @endif
