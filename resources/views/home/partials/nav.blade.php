                    ...
                    @if(Route::has('login'))
                        @auth
                            <div class="relative" x-data="{ open: false }">
                                <button @click.stop="open = !open" class="flex items-center gap-2 text-black hover:text-black transition-opacity">
                                ...
                                <div x-show="open" x-cloak @click.outside="open=false"
                                    ...
                                    <a href="{{ route('user.profile') }}" class="flex items-center gap-2 px-4 py-3 text-sm text-black hover:bg-gray-50 transition-colors">
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit" class="flex items-center gap-2 w-full px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors text-left">
        ...
        <div class="absolute top-0 right-0 h-full w-80 max-w-[85vw] bg-white shadow-2xl flex flex-col"
            ...
            <div class="px-6 py-4 bg-gradient-to-br from-gray-50 to-white border-b border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm text-black truncate">{{ Auth::user()->name }}</p>
                        <p class="text-sm text-black truncate">{{ Auth::user()->email }}</p>
                    </div>
                </div>
            </div>
            @endauth
            ...
            <nav class="flex-1 overflow-y-auto py-2">
                ...
                @foreach($drawerLinks as $i => $link)
                    <a href="{{ $link['url'] }}"
                       @click="mobileOpen = false"
                       class="flex items-center gap-4 px-6 py-4 text-sm text-black hover:text-[#63360D] hover:bg-orange-50/50 transition-all duration-200 border-b border-gray-50 group"
                       ...
                       x-transition:enter-end="opacity-100 translate-x-0">
                        ...
                        <span class="material-symbols-outlined text-base text-white group-hover:text-[#63360D] group-hover:translate-x-1 transition-all duration-200">chevron_right</span>
                    </a>
                @endforeach
                ...
                <div class="px-6 py-3 bg-gray-50">
                    <p class="text-sm text-black mb-3">Quick actions</p>
                    <div class="space-y-2">
                        <a href="{{ route('cart.show') }}"
                           @click="mobileOpen = false"
                           class="flex items-center gap-3 px-4 py-3 text-sm text-black bg-white hover:bg-orange-50 border border-gray-200 hover:border-[#63360D] transition-all duration-200 group">
                            <span class="material-symbols-outlined text-xl text-[#63360D]">shopping_bag</span>
                            <span>View Cart</span>
                        </a>
                        <button @click="weatherOpen = true; mobileOpen = false"
                                class="w-full flex items-center gap-3 px-4 py-3 text-sm text-black bg-white hover:bg-orange-50 border border-gray-200 hover:border-[#63360D] transition-all duration-200 group">
            ...
            <div class="px-6 py-5 border-t border-gray-200 bg-gradient-to-br from-gray-50 to-white space-y-3">
                ...
                @guest
                    <div class="flex gap-2">
                        <a href="{{ url('login') }}" 
                           class="flex-1 text-center py-3 text-sm border-2 border-gray-300 text-black hover:border-[#964B00] hover:text-[#964B00] transition-all duration-200">
                            Login
                        </a>
                        <a href="{{ url('register') }}" 
                           class="flex-1 text-center py-3 text-sm text-white bg-gray-900 hover:bg-gray-800 transition-all duration-200">
                            Register
                        </a>
                    </div>
                @else
                ...
                <div class="flex items-center justify-center gap-2 pt-2">
                    ...
                    <p class="text-[10px] text-black">Tambobong, Dasol, Pangasinan</p>
                </div>
            </div>
        </div>
    </div>
