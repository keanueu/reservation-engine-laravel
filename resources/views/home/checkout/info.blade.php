<div class="bg-white p-8 border border-gray-300 shadow-md space-y-6">
    <h2 class="text-xl font-normal tracking-wide text-black border-b border-gray-300 pb-3">
        Important Information
    </h2>

    <div>
        <p class="text-sm font-normal text-black mb-2">Reservation & Cancellation Policy</p>
        <button type="button"
            class="px-4 py-2 text-xs  text-white bg-[#964B00] border border-[#964B00] hover:bg-[#7a3c00] transition duration-150  shadow-sm"
    ...
    @foreach($cartRooms as $room)
        <div class="bg-white p-4 border border-gray-300 ">
            ...
            <div class="flex justify-between  text-sm text-gray-800">
                <div>
                    <p class=" text-xs text-black">Check-in Time</p>
                    <span class="text-green-600 text-xs tracking-wide">
                        after {{ \Carbon\Carbon::parse($room->check_in)->format('h:i A') }}
                    </span>
                </div>
                <div class="text-right">
                    <p class=" text-xs text-black">Check-out Time</p>
                    <span class="text-red-600 text-xs tracking-wide">
...
<div id="policy-modal"
    class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center hidden z-50 transition-opacity font-[Manrope]"
    role="dialog" aria-modal="true" tabindex="-1">
    <div class="bg-white w-full max-w-lg mx-4 p-6 relative shadow-2xl transform transition-transform duration-300">
        <button type="button" class="absolute top-4 right-4 text-gray-300 hover:text-black text-3xl  p-1"
            data-overlay="#policy-modal" title="Close">
            &times;
        </button>

        <h3 class="text-2xl font-normal mb-4 border-b pb-2 text-black font-[Manrope]">Hotel Policy & Terms</h3>
