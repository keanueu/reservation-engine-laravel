<div x-show="tab === 'boats'" x-transition>
  ...
  <div class="max-w-4xl font-[Manrope] mx-auto mb-10">
    ...
    <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-white p-6 border border-gray-200 shadow-sm">
      <div>
        <label for="booking_date" class="block text-sm font-normal text-gray-600 mb-1">Date</label>
      ...
      <div>
        <label for="guests" class="block text-sm font-normal text-gray-600 mb-1">Guests</label>
      ...
      <div>
        <label for="start_time" class="block text-sm font-normal text-gray-600 mb-1">Start Time</label>
        <input id="start_time" type="time" name="start_time" x-model="boat_start_time"
          class="mt-1 block w-full border-gray-300 shadow-sm sm:text-sm h-11 px-3" required>
      </div>
      <div>
        <label for="end_time" class="block text-sm font-normal text-gray-600 mb-1">End Time</label>
      ...
      <div class="group bg-white border border-gray-200 shadow-sm overflow-hidden flex flex-col md:flex-row h-full">
        <div class="relative overflow-hidden h-56 md:h-auto md:w-2/5">
          ...
          <div class="absolute bottom-4 left-4">
            ...
            <span class="text-sm  text-gray-200 tracking-wide">PER TRIP</span>
          </div>
        </div>
        ...
        <div class="p-4 flex flex-col flex-grow md:w-3/5">
          ...
          <div class="flex flex-wrap gap-x-4 gap-y-3 text-gray-600 text-xs  mb-4 pt-3 border-t border-gray-200">
            ...
            <div class="flex items-center gap-2" title="Capacity">
              <svg class="w-4 h-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M17 20v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2m8-10a4 4 0 100-8 4 4 0 000 8z" />
              </svg>
              <span class=" text-xs text-gray-600">{{ $boat->capacity }} Persons</span>
            </div>
            ...
            <div class="flex items-center gap-2" title="Available Start Time">
              ...
              <span class=" text-xs text-gray-600">Starts:
            ...
            <div class="flex items-center gap-2" title="Available End Time">
              ...
              <span class=" text-xs text-gray-600">Ends:
                {{ $boat->end_time ? \Carbon\Carbon::parse($boat->end_time)->format('h:i A') : 'N/A' }}</span>
            </div>
          </div>
          <div class="mt-auto flex items-center justify-between">
            <a href="{{ url('boat_details', $boat->id) }}"
              class="text-[#964B00] underline text-xs hover:text-yellow-600">
              View Details
            </a>

            <button data-boat-id="{{ $boat->id }}"
              @click.prevent="submitBoatBooking({{ $boat->id }}, {{ $boat->capacity }})"
              class="book-now-btn bg-[#964B00] px-6 py-2.5 text-xs text-white hover:bg-black  transition flex items-center justify-center tracking-wide">
