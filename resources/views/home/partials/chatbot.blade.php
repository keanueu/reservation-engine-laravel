    ...
    <!-- Tailwind is required for the responsiveness -->

    <!-- Chat Floating Action Button (FAB) -->
    <button id="chat-fab"
        class="fixed bottom-4 right-4 inline-flex items-center justify-center text-sm font-medium disabled:pointer-events-none disabled:opacity-50 border  w-16 h-16 bg-[#964B00] hover:bg-black m-0 cursor-pointer border-gray-200 p-0 normal-case leading-5 hover:text-gray-900 z-50 shadow-xl transition-transform duration-300 hover:scale-105"
    ...
    <!-- Main Chat Container - FULLY RESPONSIVE CLASSES -->
    <div id="ai-chat-container" 
        style="box-shadow: 0 0 #0000, 0 0 #0000, 0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1);"
        class="
            /* Mobile: Full-screen takeover */
            fixed inset-0 w-full h-full bg-white p-4 rounded-none z-50 flex flex-col transition-all duration-300

            /* Desktop/Tablet (md breakpoint): Fixed widget positioning and size */
            md:bottom-[calc(4rem+1.5rem)] md:right-4 md:w-96 md:h-[35rem] md:max-h-[80vh] md:mr-0 md:p-6 md: md:top-auto md:left-auto
            
            hidden-chat ai-chat-closed border border-[#e5e7eb]
        ">

        <!-- Close Button (Absolute position) -->
        <button id="close-chat-btn" class="absolute top-4 right-4 text-gray-400 hover:text-red-600 md:top-6 md:right-6 transition-colors">
        ...
        <!-- Chat Header -->
        <div class="flex flex-row items-center space-x-3 pb-4 border-b border-gray-300">
            <div class="relative flex items-center justify-center w-10 h-10  border border-black shrink-0">
                <!-- AI Icon SVG -->
                <svg fill="none" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="0 0 512 512">
            ...
            <div class="flex flex-col">
                <h2 class="font-semibold text-lg text-black">Your Booking Assistant</h2>
                <p class="text-xs text-black">Powered by Meta-Qwen 2.5:3b</p>
            </div>
        </div>


        <!-- Chat History Box (Flex-1 to take up dynamic height) -->
        <div id="chat-box" class="pr-2 pt-4 flex-1 overflow-y-auto flex flex-col space-y-4">
            
            <!-- Initial Message (Bot) -->
            <div class="flex gap-3 text-gray-600 text-sm">
                <span class="relative flex shrink-0 overflow-hidden  w-8 h-8 mt-1">
                    <div class=" bg-gray-100 border p-1 shrink-0">
                        <!-- Bot Icon -->
                ...
                <p class="leading-relaxed bg-gray-100  p-3 max-w-[85%] text-gray-800">
                    <span class=" text-sm text-black">Hi, how can I help you today? </span>
                </p>
            </div>
        ...
        <!-- Input and Footer Section -->
        <div class="flex flex-col border-t border-gray-300 pt-4 mt-4 shrink-0">
            <form id="chat-form" class="flex items-center w-full space-x-2">
                <input id="chat-input"
                    class="flex h-10 w-full border border-gray-400 px-3 py-2 text-sm placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#964B00] focus:border-transparent"
                    placeholder="Type your message..." value="">
                <button id="send-btn" type="submit"
                    class="inline-flex items-center justify-center text-sm text-white disabled:pointer-events-none disabled:opacity-50 bg-[#964B00] hover:bg-black h-10 px-4 py-2 transition-colors">
                    Send
                </button>
            </form>

            <p class="mt-2 text-xs text-black text-center">
                Ask me about availability, booking, rooms, etc.
            </p>
        </div>
    </div>
